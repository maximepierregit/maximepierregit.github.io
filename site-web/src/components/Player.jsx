...
export default function Player() {
  ...
  const muteToggle = () => {
    dispatch({ type: ACTIONS.MUTE_TOGGLE });
  };

  // TODO : ajouter une action d'activer ou désactiver le mode "shuffle"
  const shuffleToggle = () => {
    dispatch({ type: ACTIONS.SHUFFLE_TOGGLE });
  };
    ...
    <>
      ...
      <div id="controls" className="flex-column">
        ...
        <section id="timeline-container" className="flex-row">
          ...
          <span id="timeline-current" >{state.audio.currentTime ? formatTime(state.audio.durcurrentTimeation) : "{0:00}"}</span>
